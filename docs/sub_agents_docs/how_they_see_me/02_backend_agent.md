...
3. **핵심 로직**:
    ...
    - **결과 계신**: Johari Window 4개 영역(Open, Blind, Hidden, Unknown) 데이터 가공
...
- **Rate Limit (`express-rate-limit`)**:
    ...
    - `answerLimiter`: IP당 10회/분, ShareId당 3회/분 (응답 제출)
...
- **Logic**:
    ...
    2. **Whitelist 검증**: `selfKeywords`가 서버 정의 리스트에 포함되는지 확인
    3. **ShareId 생성**: `nanoid` 또는 `base62` (10자 이상)
    4. **DB 저장**: `JohariTest` 문서 생성
- **Output**: `{ shareId: "..." }`
...
- **Logic**:
    ...
    2. **중복 검사 (3중)**:
        - `fingerprintHash`가 해당 `testId`의 `JohariAnswer`에 이미 존재하는지 확인
        - 동일 IP + `testId` 조합 확인 (최근 N분 내)
        - 동일 UserAgent + `testId` 확인
    3. **저장 및 업데이트**:
        - `JohariAnswer` 저장
