#!/bin/bash

# --- 설정 변수 ---
REMOTE_USER="ksh"
REMOTE_HOST="192.168.0.6"
REMOTE_DIR="~/srv/claw-addict-client"
LOCAL_CLIENT_ROOT="."

echo "🌐 홈서버($REMOTE_HOST)로 클라이언트 단독 배포를 시작합니다..."

# 0. 서버에 디렉토리가 없으면 생성
ssh $REMOTE_USER@$REMOTE_HOST "mkdir -p $REMOTE_DIR"

# 1. 로컬 클라이언트 파일 서버로 동기화
echo "📤 파일 동기화 중 (rsync)..."
rsync -avz --progress \
  --exclude 'node_modules' \
  --exclude '.next' \
  --exclude '.git' \
  $LOCAL_CLIENT_ROOT/ $REMOTE_USER@$REMOTE_HOST:$REMOTE_DIR

# 2. 서버에서 빌드 및 실행 명령 전달
echo "🛠️ 서버에서 빌드 및 PM2 실행 중..."
# NVM을 확실하게 로드하기 위해 대중적인 NVM 경로들을 모두 확인하여 소싱합니다.
ssh $REMOTE_USER@$REMOTE_HOST "
  export NVM_DIR=\"\$HOME/.nvm\"
  [ -s \"\$NVM_DIR/nvm.sh\" ] && . \"\$NVM_DIR/nvm.sh\"
  
  # nvm이 없다면 시스템 노드라도 확인 (버전 이슈 해결용)
  if command -v nvm >/dev/null 2>&1; then
    nvm use 20
  fi
  
  echo \"사용중인 Node 버전: \$(node -v)\"
  
  cd $REMOTE_DIR && \
  npm install && \
  npm run build && \
  # PM2 실행 (기존 프로세스가 있으면 유지하고 리로드, 없으면 새로 시작)
  pm2 reload claw-addict-fe --update-env || pm2 start ecosystem.config.js
"

echo "✅ 배포 완료! https://claw-addict-web.haroo.site 주소를 확인하세요."
